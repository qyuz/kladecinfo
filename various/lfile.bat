...
for %%a in (%file%) do (
	...
	c:\p\ffmpeg\bin\ffmpeg -y -i "%sourcePath%/%%~a" -codec:v libx264 -profile:v high -crf 23 -preset ultrafast -codec:a copy -vbr 4 -movflags faststart -vf scale=-1:540,format=yuv420p -f segment -segment_time 1710.65 -segment_start_number 1 -reset_timestamps 1 "%tempFolder%\%%~na_%%02d.mp4" 
